...
###########################
'''
from mailroom4 import send_a_thankyou
from mailroom4 import create_a_report
from mailroom4 import send_letters_to_all_donors
...
def test_send_a_thankyou1():
    ...
    assert send_a_thankyou('Paul Allen', 100, donor_db) == expected

def test_send_a_thankyou2():
    '''send a thankyou note to donor'''
    expected = ('Sarah Smith', [100.00, 1])
    assert send_a_thankyou('Sarah Smith', 100, donor_db) == expected

def test_create_a_report():
    '''test the text created by create_a_report'''
    expected = "Mark Zuckerberg           $      1663.23             3  $      554.41"
    assert create_a_report('Mark Zuckerberg', donor_db) == expected
...
def test_send_letters_to_all_donors():
    ...
    expected = "Dear Jeff Bezos,\n\n" \
                ...
                "                       Sincerely\n" \
                "                          -The Team"
    assert send_letters_to_all_donors('Jeff Bezos', donor_db) == expected
